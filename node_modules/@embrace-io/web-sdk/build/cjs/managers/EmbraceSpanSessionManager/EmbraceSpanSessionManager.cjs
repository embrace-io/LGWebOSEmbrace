...
var EmbraceExtendedSpan = require('../EmbraceTraceManager/EmbraceExtendedSpan.cjs');
var appInstanceId = require('../../resources/appInstanceId.cjs');
...
class EmbraceSpanSessionManager {
    constructor({ diag: diagParam, perf, visibilityDoc = window.document, storage = window.localStorage, sessionStorage = window.sessionStorage, limitManager, referrer = document.referrer }){
        ...
        this._tracer = api.trace.getTracer('embrace-web-sdk-sessions');
        this._noExportTracer = new sdkTraceWeb.BasicTracerProvider().getTracer('embrace-web-sdk-sessions');
    ...
    _endSessionSpanAttributes(reason) {
        return {
            ...
            [attributes.KEY_EMB_SDK_STARTUP_DURATION]: this._sdkStartupDuration
        };
    }
    ...
    currentSessionAsReadableSpan(reason) {
        ...
        const span = this._noExportTracer.startSpan('emb-session', {
            ...
            attributes: {
                ...
                [attributes.KEY_EMB_FROM_STORAGE]: true
            }
        });
        span.end();
        return span;
    ...
    startSessionSpan(options) {
        ...
        const attributes$1 = {
            ...
            [attributes.KEY_EMB_EXPERIENCE_ID]: this._currentTab.experienceId,
            [attributes.KEY_EMB_TAB_ID]: this._currentTab.tabId
        };
        if (this._currentTab.parentTabId) {
            attributes$1[attributes.KEY_EMB_PARENT_TAB_ID] = this._currentTab.parentTabId;
        }
        if (options?.reason) {
            attributes$1[attributes.KEY_EMB_SESSION_REASON_STARTED] = options.reason;
        }
        this._sessionSpan = new EmbraceExtendedSpan.EmbraceExtendedSpan(this._tracer.startSpan('emb-session', {
    ...
    _initTab() {
        ...
        try {
            const stored = this._sessionStorage.getItem(constants.EMBRACE_TAB_STORAGE_KEY);
            if (stored) {
        ...
        const currentTab = {
            experienceId: parentActivity?.experienceId ?? generateUUID.generateUUID(),
            parentTabId: parentActivity?.tabId,
            tabId: appInstanceId.getAppInstanceId(this._sessionStorage, this._diag)
        };
        // Persist tab data for page reloads
        try {
            this._sessionStorage.setItem(constants.EMBRACE_TAB_STORAGE_KEY, JSON.stringify(currentTab));
        } catch (e) {
    ...
    _getLastTabActivity() {
        try {
            const stored = this._storage.getItem(constants.EMBRACE_TAB_ACTIVITY_STORAGE_KEY);
            if (!stored) {
                return null;
            }
            return JSON.parse(stored);
    ...
    _saveLastTabActivity(activity) {
        try {
            this._storage.setItem(constants.EMBRACE_TAB_ACTIVITY_STORAGE_KEY, JSON.stringify(activity));
        } catch (error) {
...
//# sourceMappingURL=EmbraceSpanSessionManager.cjs.map
